...
__global__ void sparse_mat_scan(int * input, int * output_index_array, int* auxiliry_array, int input_size)
{
	int idx = threadIdx.x;
...
__global__ void sparse_mat_summation(int * output_index_array, int * auxiliry_array, int input_size)
{
	int idx = threadIdx.x;
	int gid = blockDim.x*blockIdx.x + threadIdx.x;
...
void run_sparse_matrix()
{
	...
	sparse_mat_scan << <grid, block >> > (d_input, d_output_index_array, d_aux, input_size);
	...
	sparse_mat_summation << <grid, block >> > (d_output_index_array, d_sum_aux, input_size);
	gpuErrchk(cudaDeviceSynchronize());
	...
	sparse_mat_compact << <grid, block >> > (d_input, d_output, d_output_index_array, input_size);
	gpuErrchk(cudaDeviceSynchronize());
